...
// stage content:
(lib.CR_11155_MINAJESTY_2014_US_DIGITAL_300x600_HTML5 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	...
	this.frame_0 = function() {
		...
		///* We are importing our custom AS3 class MovingClouds. The folder com.flashandmath... with its subfolders must be in the same directory as this fla file for the file to compile. */
		...
		////var bannerWidth:Number = 160;
		////var bannerHeight:Number = 600;
		
		//var bannerWidth:Number = stage.stageWidth;
		//var bannerHeight:Number = stage.stageHeight;
		
		//var speedX:Number = 3.75;
		//var speedY:Number = 5;
		
		///*
		//We are creating two instances of MovingClouds of the size bannerWidth by bannerHeight pixels.
		//The parameters -2 and 3 (or -1, 1) correspond to horizontal and vertical direction and speed of motion. So each instance, a 'layer', moves at a different speed.
		//'false' tells the constructor not to use the default blue background provided within the class. Instead, we create in this fla file a common background for both instances, blueBackground.
		//*/
		
		
		//var clouds1:MovingClouds = new MovingClouds(bannerWidth, bannerHeight, speedX, -speedY, false);
		//clouds1.x = posX;
		//clouds1.y = posY;
		//clouds1.alpha = .5;
		
		//var clouds2:MovingClouds = new MovingClouds(bannerWidth, bannerHeight, -speedX/4, -speedY/3, false);
		//clouds2.x = posX;
		//clouds2.y = posY;
		//clouds2.alpha = .75;
		...
		///*
		//The text Flash&Math has been created on the stage, stored in the Library and linked to AS3 under the name mcText.
		//*/
		
		////var mc:mc_clouds = new mc_clouds();
		////mc.x = 300;
		////mc.y = 200;
		////
		...
		//mc_empty1.addChild(clouds1);
		//mc_empty2.addChild(clouds2);
		////mc_empty.addChild(mc);
		...
		//var animDuration02:Number = 4;
		//var animDuration03:Number = 2;
		
		//var animDelayFR1:Number = .5;
		//var animDelayFR2:Number = animDelayFR1 + animDuration01 + .25;
		//var animDelayFR3:Number = animDelayFR2 + animDuration02 + 1.5;
		...
		////TweenLite.to(mc_image, animDuration02, {alpha:1, delay:animDelayFR1, ease:Cubic.easeOut});
		////TweenLite.to(mc_logo, animDuration02, {alpha:1, delay:animDelayFR1 + .25, ease:Cubic.easeOut});
		////TweenLite.to(mc_bottle, animDuration02, {alpha:1, delay:animDelayFR1 + .5, ease:Cubic.easeOut});
		...
		// 
		//function init():void{
		//   glow = new GlowFilter(0xffb4f7,1,5,5,2,BitmapFilterQuality.HIGH); 
		//   mc_logo.filters = [glow];
		//   mc_logo.alpha = 0;
		////   timerOn = new Timer(1000);
		////   timerDuration = new Timer(1000);
		//   timerOn = new Timer(animDelayFR1 * 1000);
		//   timerDuration = new Timer(animDuration01 * 1000);
		...
		//TweenLite.to(mc_empty1, animDuration02, {alpha:.75, delay:animDelayFR2, ease:Cubic.easeOut});
		
		//TweenLite.to(mc_image, animDuration02, {alpha:1, delay:animDelayFR2, ease:Cubic.easeOut});
		//TweenLite.to(mc_logo, animDuration02, {alpha:0, delay:animDelayFR2, ease:Cubic.easeOut});
		//TweenLite.to(mc_bottle, animDuration02, {alpha:1, delay:animDelayFR2 + .5, ease:Cubic.easeOut});
		//TweenLite.to(mc_tag, animDuration02, {alpha:1, delay:animDelayFR2 + .5, ease:Cubic.easeOut});
		//TweenLite.to(btn_shop, animDuration01, {alpha:1, delay:animDelayFR2 + 2, ease:Cubic.easeOut});
		...
		////	loadMovie("CR_11155_MINAJESTY_2014_US_DIGITAL_728x90b.swf", mc_loadSWF);
		//////	loadMovie("img/CR_11155_MINAJESTY_2014_US_DIGITAL_728x90b.swf", mc_loadSWF);
		////}
		...
		////	var url:URLRequest = new URLRequest("CR_11155_MINAJESTY_2014_US_DIGITAL_728x90b.swf");
		//////	var url:URLRequest = new URLRequest("img/CR_11155_MINAJESTY_2014_US_DIGITAL_728x90b.swf");
		////
		////	swfLoader.load(url);
